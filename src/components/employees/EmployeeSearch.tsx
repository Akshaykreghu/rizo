      ...
      {open && options.length > 0 && (
        <div className="absolute z-20 mt-1.5 w-full bg-white border border-slate-200 rounded-lg shadow-lg max-h-64 overflow-y-auto scroll-fade">
          {options.map((emp) => (
            ...
            >
              <Avatar name={`${emp.first_name} ${emp.last_name}`} imageUrl={emp.profile_pic} className="w-7 h-7 flex-shrink-0 text-[10px]" />
              <span className="min-w-0 flex-1">
                <span className="block truncate text-[#0F172A] font-medium">{emp.first_name} {emp.last_name}</span>
