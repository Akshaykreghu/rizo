...
// Ports EditPunchesController::Iterateame() — pulls raw device_attandance punches into the derived
// emp_detail_timeattandance daily rows for the month, via the confirmed-live device_logs_iteration_fn
// (keyed by emp_id, the varchar company id, not emp_pkey).
...
export async function POST(request: NextRequest) {
  ...
  const [[result]] = await pool.query<RowDataPacket[]>(
    'SELECT device_logs_iteration_fn(?, ?) AS r',
    [emp.emp_id, `${month}-01`]
  );
