...
function ManualGrid({
  ...
  // From Date, To Date, or either half changes — same query the admin Leave Requests page's Apply
  // Leave form uses, since this Apply Leave button is the same action, just reachable from this page.
  ...
  // Same endpoint as the admin Leave Requests page's own Apply Leave form (/api/leave/requests) —
  // this button is the same action, not a separate bulk-upload-specific apply path, per an explicit
  // "call the same Apply Leave" decision: the two pages must not diverge in what applying a single
  // leave actually does.
  const add = useMutation({
    ...
    // Server-state hard block (leave already exists/attendance verified/punch conflict/no balance)
    // discovered at submit time — same alert-popup treatment as the admin Leave Requests page, not
    // inline form text or a toast (toasts are reserved for the outcome of an action that actually ran).
    onError: (err: Error) => setAlertMessage(err.message),
  });
