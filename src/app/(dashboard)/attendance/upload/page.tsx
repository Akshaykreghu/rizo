...
export default function AttendanceUploadPage() {
  ...
  const addSingle = useMutation({
    mutationFn: () =>
      fetch('/api/attendance/upload/single', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          empFkey: Number(addForm.empFkey),
          date: addForm.date,
          inTime: addForm.inTime,
          outTime: addForm.outTime,
        }),
      }).then(async (r) => {
        const body = await r.json();
        if (!r.ok) throw new Error(body.error ?? 'Failed to save attendance');
        return body;
      }),
    onSuccess: () => {
      setShowAdd(false);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/25 backdrop-blur-[2px] p-4 animate-fade-in" onClick={() => setShowAdd(false)}>
          ...
          >
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-[19px] font-semibold text-[#0F172A] tracking-tight">Add Attendance</h2>
            ...
            <div className="space-y-3">
              <div>
                ...
                <SearchableSelect
                  value={addForm.empFkey}
                  onChange={(v) => setAddForm((f) => ({ ...f, empFkey: v }))}
                  options={employeeOptions}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[12px] font-medium text-slate-600 mb-1.5">In Time</label>
                  <TimePicker value={addForm.inTime} onChange={(v) => setAddForm((f) => ({ ...f, inTime: v }))} />
                </div>
                <div>
                  <label className="block text-[12px] font-medium text-slate-600 mb-1.5">Out Time</label>
                  <TimePicker value={addForm.outTime} onChange={(v) => setAddForm((f) => ({ ...f, outTime: v }))} />
                </div>
              </div>
              {addError && (
            ...
            <button
              ...
              disabled={!addForm.empFkey || !addForm.date || !addForm.inTime || !addForm.outTime || addSingle.isPending}
