...
function GenderSelect({ form, onChange }: { form: FormState; onChange: (e: { target: { name: string; value: string } }) => void }) {
  return (
    <div>
      ...
      <select value={form.classification ?? ''} onChange={(e) => onChange({ target: { name: 'classification', value: e.target.value } })} style={INP}>
        <option value="">—</option>
...
export default function EssAboutPage() {
  ...
  async function addFamily() {
    ...
    try {
      await fetch(`/api/employees/${empId}/family`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(familyDraft),
      });
  ...
  async function addDocument() {
    ...
    try {
      await fetch(`/api/employees/${empId}/documents`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(docDraft),
      });
  ...
  const timeline: TLItem[] = (() => {
    ...
    return items.sort((a, b) => (b.date ? new Date(b.date).getTime() : 0) - (a.date ? new Date(a.date).getTime() : 0));
  })();
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 310px', gap: 22, alignItems: 'start' }}>
        ...
        <div>
          ...
          <div style={{ background: 'var(--bg-card)', border: '1.5px solid var(--border)', borderRadius: 16, overflow: 'hidden', marginBottom: 20, position: 'relative' }}>
            ...
            <div style={{ padding: '14px 24px 20px 140px', minHeight: 72 }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginTop: 8 }}>
                ...
                {emp.emp_id && <span style={{ padding: '2px 10px', borderRadius: 20, fontSize: 11, fontWeight: 600, background: 'var(--border)', color: 'var(--text-muted)', fontFamily: 'monospace' }}>{emp.emp_id}</span>}
              </div>
            </div>
          </div>
          ...
          <div style={{ background: 'var(--bg-card)', border: '1.5px solid var(--border)', borderRadius: 16, padding: '16px 20px', marginBottom: 20 }}>
            ...
            <G>
              <F label="Employee Code" value={emp.emp_id} />
                    ...
                    {showFamilyForm ? (
                      <div style={{ padding: '12px 14px', background: 'rgba(30,81,110,0.04)', borderRadius: 10, marginBottom: 8, border: '1.5px dashed rgba(30,81,110,0.3)' }}>
                        ...
                        <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                          <button onClick={addFamily} disabled={familySaving} style={{ padding: '5px 14px', background: '#1E516E', color: '#fff', border: 'none', borderRadius: 6, fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>{familySaving ? 'Adding…' : 'Add'}</button>
                          <button onClick={() => { setShowFamilyForm(false); setFamilyErrors({}); }} style={{ padding: '5px 12px', border: '1.5px solid var(--border)', borderRadius: 6, background: 'transparent', color: 'var(--text-muted)', fontSize: 12, cursor: 'pointer' }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
                      <button onClick={() => setShowFamilyForm(true)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', border: '1.5px dashed rgba(30,81,110,0.35)', borderRadius: 8, background: 'transparent', color: '#1E516E', fontSize: 12, fontWeight: 700, cursor: 'pointer', width: '100%', justifyContent: 'center' }}>
                        + Add Family Member
                      </button>
                    )}
                  </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  ...
                  {editing && (showDocForm ? (
                    <div style={{ padding: '12px 14px', background: 'rgba(30,81,110,0.04)', borderRadius: 10, border: '1.5px dashed rgba(30,81,110,0.3)' }}>
                      ...
                      <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                        <button onClick={addDocument} disabled={docSaving} style={{ padding: '5px 14px', background: '#1E516E', color: '#fff', border: 'none', borderRadius: 6, fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>{docSaving ? 'Adding…' : 'Add'}</button>
                        <button onClick={() => { setShowDocForm(false); setDocErrors({}); }} style={{ padding: '5px 12px', border: '1.5px solid var(--border)', borderRadius: 6, background: 'transparent', color: 'var(--text-muted)', fontSize: 12, cursor: 'pointer' }}>Cancel</button>
                      </div>
                    </div>
                  ) : (
                    <button onClick={() => setShowDocForm(true)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', border: '1.5px dashed rgba(30,81,110,0.35)', borderRadius: 8, background: 'transparent', color: '#1E516E', fontSize: 12, fontWeight: 700, cursor: 'pointer', width: '100%', justifyContent: 'center' }}>
                      + Add Document
                    </button>
                  ))}
                </div>
